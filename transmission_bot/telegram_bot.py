...
/delete <Id Torrent> remove torrent
"""
...
def delete_command(bot, update):
    ...
    try:
        global_broker.remove_torrent(
            update.message.chat_id, update.message.text.split(' ', 1)[1])

        bot.sendMessage(chat_id=update.message.chat_id,
                        text="Torrent successfully deleted")
    except TransmissionError as e:
        transmission_error(bot, update, e)
...
def free_space_command(bot, update):
    try:
        free_space = global_broker.free_space(update.message.chat_id)
        bot.sendMessage(chat_id=update.message.chat_id,
                        text=f"Here are current free device space:\n\n{free_space} GB")
    except TransmissionError as e:
        transmission_error(bot, update, e)
...
def start_bot():
    ...
    free_space_handler = CommandHandler('free_space', free_space_command)
